...
def wikipedia_links_en(indir: str=os.path.join(DBPEDIA_DIR, "cores/wikipedia_links_en"), infile: str="step_0/wikipedia-links_lang=en.ttl.bz2"):
    ...
    if not os.path.exists(step_1_outfile):
        ...
        rdd = rdd.filter(lambda triple: triple[1] == "http://xmlns.com/foaf/0.1/isPrimaryTopicOf")
        rdd.map(lambda x: orjson.dumps(x).decode()).saveAsTextFile(step_1_outfile, compressionCodecClass="org.apache.hadoop.io.compress.GzipCodec")
    else:
...
def instance_types_specific_en(indir: str=os.path.join(DBPEDIA_DIR, "cores/instance_types_specific_en"), infile: str="step_0/instance-types_lang=en_specific.ttl.bz2"):
    ...
    if not os.path.exists(step1_outfile):
        ...
        rdd.map(orjson.dumps).saveAsTextFile(step1_outfile, compressionCodecClass="org.apache.hadoop.io.compress.GzipCodec")
...
def fusion_instance_types_en(indir: str=os.path.join(DBPEDIA_DIR, "fusion/instance_types_en"), infile: str="step_0/instance-types_reduce=dbpw_resolve=union.ttl.bz2"):
    ...
    print(rdd.filter(lambda x: x[0] == 'http://dbpedia.org/resource/Stahlbahnwerke_Freudenstein').take(1))
    
    return rdd


def mappingbased_literals_en(indir: str=os.path.join(DBPEDIA_DIR, "cores/mappingbased_literals_en"), infile: str="step_0/mappingbased-literals_lang=en.ttl.bz2"):
    sc = get_spark_context()

    step1_outfile = os.path.join(indir, "step_1")
    if not os.path.exists(step1_outfile):
        # convert triples
        rdd = sc.textFile(os.path.join(indir, infile), minPartitions=32, use_unicode=False)
        rdd = rdd.map(ntriple_loads).map(lambda x: (str(x[0]), x)) \
            .groupByKey().map(lambda x: list(x[1])).map(merge_triples)
...
def mappingbased_objects_en(indir: str=os.path.join(DBPEDIA_DIR, "cores/mappingbased_objects_en"), infile: str="step_0/mappingbased-objects_lang=en.ttl.bz2"):
    sc = get_spark_context()
    ...
    if not os.path.exists(step1_outfile):
        # convert triples
        rdd = sc.textFile(os.path.join(indir, infile), minPartitions=32)
        rdd = rdd.filter(ignore_comment).map(ntriple_loads) \
...
def redirects_en(indir: str=os.path.join(DBPEDIA_DIR, "cores/redirects_en"), infile: str="step_0/redirects_lang=en.ttl.bz2"):
    sc = get_spark_context()
...
def pages_en(indir: str=os.path.join(DBPEDIA_DIR, "cores/pages_en"), infile: str="step_0/page_lang=en_ids.ttl.bz2"):
...
def merge_instances_en(instance_types_rdd=None, mappingbased_literals_rdd=None, mappingbased_objects_rdd=None, pages_rdd=None, outfile: str=os.path.join(DBPEDIA_DIR, "instances_en/step_0")):
    ...
    if not os.path.exists(outfile):
        ...
        rdd2 = (instance_types_rdd or instance_types_specific_en()).map(lambda x: (x[0], x))

        rdd3 = (mappingbased_literals_rdd or mappingbased_literals_en()).map(lambda x: (x['@id'], x))
        rdd4 = (mappingbased_objects_rdd or mappingbased_objects_en()).map(lambda x: (x['@id'], x))

        rdd234 = rdd2 \
            .leftOuterJoin(rdd3).mapValues(merge_triple_to_json) \
            .leftOuterJoin(rdd4).mapValues(merge_json_objects)
        
        rdd = rdd1.leftOuterJoin(rdd234).mapValues(merge_json_objects).map(itemgetter(1))
        rdd.map(orjson.dumps).saveAsTextFile(outfile, compressionCodecClass="org.apache.hadoop.io.compress.GzipCodec")
        ensure_unique_records(rdd, itemgetter('@id'))
    else:
...
def merged_instances_fixed_wiki_id_en(merge_instances_rdd=None, id2groups_rdd=None, outfile: str=os.path.join(DBPEDIA_DIR, "instances_en/step_1")):
    ...
    if not os.path.exists(outfile):
        ...
        def process_join_result(x):
            ...
            instance['http://dbpedia.org/ontology/wikiPageID'] = wiki_article['final'][1]
            return instance 

        merge_instances_rdd \
            .flatMap(lambda x: [(wiki_id, x) for wiki_id in get_wiki_page_ids(x)]) \
            .join(id2groups_rdd) \
            .map(process_join_result) \
            .map(orjson.dumps) \
