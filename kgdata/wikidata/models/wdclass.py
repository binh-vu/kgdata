...
@dataclass
class WDClass:
    __slots__ = ("id", "label", "description", "datatype", "aliases", "parents", "properties", "different_froms", "equivalent_classes", "parents_closure")
    id: str
    ...
    @staticmethod
    def from_file(indir: Union[str, Path] = os.path.join(WIKIDATA_DIR, "ontology"), load_parent_closure: bool = False) -> Dict[str, 'WDClass']:
        ...
        if load_parent_closure:
            ...
            parents_closure = deserialize_jl(os.path.join(indir, "superclasses_closure.jl"))
    ...
    @staticmethod
    def iter_file(indir: Union[str, Path] = os.path.join(WIKIDATA_DIR, "ontology"), load_parent_closure: bool = False) -> Generator['WDClass', None, None]:
        if load_parent_closure:
            with open(os.path.join(indir, "classes.jl"), "r") as f, open(os.path.join(indir, "superclasses_closure.jl"), "r") as g:
                while True:
                    try:
                        ...
                        try:
                            ...
                            break
                        raise Exception("Invalid superclasses_closure file. Number of rows doesn't match with the classes file")
