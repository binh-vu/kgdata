...
def test_left_outer_join():
    ...
    tmpfile = "/workspace/data/sm-dev/tmpfile"
    ...
    left_outer_join(rdd1, rdd2, rdd1_keyfn, rdd1_fk_fn, rdd2_keyfn,
                    rdd1_join_fn, rdd1_serfn, tmpfile)

    result = sc.textFile(tmpfile).map(orjson.loads).collect()
    assert sorted(result, key=rdd1_keyfn) == [
        {
            "department": 1,
            "employees": ["Peter", "Bob", "John", "Marry"]
        },
        {
            "department": 2,
            "employees": ["Marry", "Jessie"]
        },
    ]

    shutil.rmtree(tmpfile)

    left_outer_join_broadcast(rdd1, rdd2, rdd1_fk_fn, rdd2_keyfn, rdd1_join_fn,
                              rdd1_serfn, tmpfile)
